...
- `.github/workflows/release.yml`：只在推送 `vX.Y.Z` tag 或手工指定已有 tag 时运行。
  Linux 生成 `.deb` 和 `.tar.gz`，其中 `.deb` 由仓库内的二进制打包脚本直接组装；Windows
  生成 NSIS `.exe` 安装器和 `.zip`，最后生成
...
先按目标平台配置 release 和 GUI。Linux 归档仍可使用 Xpack，Debian 包必须使用独立脚本：
...
26.04 上打包前安装 `dpkg-dev` 和 `patchelf`。`package_deb` phony target 负责从当前 Xmake
配置读取 CLI、GUI 与 ilias 的精确位置；需要改变输出目录时，在配置阶段传入
`--deb_outputdir=/path/to/dist`，并与 release/GUI 的完整配置项放在同一条 `xmake f`
命令中。Windows 可使用 `-f nsis,zip`；
`lua/pack.lua` 同时绑定 `mksync` 和 `mksync-gui`，Windows 上 Xmake 的 `qt.quickapp` 安装钩子会运行
