...
- `.github/workflows/release.yml`：只在推送 `vX.Y.Z` tag 或手工指定已有 tag 时运行。
  Linux 生成 `.deb` 和 `.tar.gz`，其中 `.deb` 默认只自带项目私有库；Windows 生成精简
  NSIS `.exe` 安装器和完整 `.zip` 绿色包，最后生成
...
发布 job 同时支持两种状态：目标 Release 不存在时创建 Release 并自动生成说明；目标
Release 已经存在时保留其标题、说明、draft/prerelease 状态，并用本次构建结果覆盖同名附件。
这也兼容先在 GitHub 页面创建 Release、再由新 tag 触发构建的流程。正式版本发布后仍推荐
增加版本并创建新 tag，避免悄悄替换用户已经下载的二进制文件。

## 本地打包

先按目标平台配置 release 和 GUI。Debian 包使用独立脚本：

```bash
xmake f -c -m release --stdcxx=23 --enable_gui=y --enable_tests=n
xmake package_deb
# xmake package_deb --bundle-qt  # 自带 Qt/QML 的较大变体
```

`packaging/debian/build.sh` 使用 `dpkg-shlibdeps` 计算运行时依赖并直接调用 `dpkg-deb`，不会
运行 `debuild`、`dpkg-buildpackage`，也不会要求发行版提供可构建本项目的 Xmake。Ubuntu
26.04 上打包前安装 `dpkg-dev` 和 `patchelf`。`package_deb` 任务负责从当前 Xmake
配置读取 CLI、GUI 与 ilias 的精确位置；需要改变本次输出目录时传入
`--output-dir=/path/to/dist`。Windows 可使用 `-f nsis,zip`；
`lua/pack.lua` 同时绑定 `mksync` 和 `mksync-gui`。Windows 的 NSIS 格式向 `windeployqt`
追加 `--no-compiler-runtime`、`--no-system-d3d-compiler` 和 `--no-opengl-sw`，ZIP 不追加这些
裁剪参数。Linux `.deb` 默认只携带项目私有库；Qt/QML/QPA、XCB、Wayland、libei/libportal、
编译器运行库、glibc 和图形库由目标系统提供，最低 ABI 版本写入 `Depends`。打包时追加
`--bundle-qt` 可生成自带 Qt 的较大变体。当前没有 macOS 输入后端，因此 Release
暂不发布无实际功能的 macOS 安装包。
