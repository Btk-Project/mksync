...
- `.github/workflows/ci.yml`：每次推送到 `main` 或提交 PR 时运行。Linux C++23
  固定在 Ubuntu 24.04/Clang 19/libstdc++ 14，使用系统 Qt 6、fmt、spdlog 和 gtest 构建 CLI、GUI 并
...
Ubuntu 24.04 环境可使用 `-f deb`，Windows 可使用 `-f nsis,zip`。`lua/pack.lua` 同时绑定
`mksync` 和 `mksync-gui`：Windows 上 Xmake 的 `qt.quickapp` 安装钩子会运行
`windeployqt`；macOS 会打包 Qt `.app`；Linux 使用 Ubuntu 24.04 系统 Qt，QML 导入模块、
