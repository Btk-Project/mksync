...
namespace mks::base
{
    ...
    struct Size {
        ...
        auto cast() const
        {
            if constexpr (std::numeric_limits<T>::min() <= std::numeric_limits<G>::min() &&
                ...
                if (width < std::numeric_limits<G>::min() ||
                    width > std::numeric_limits<G>::max() ||
                    height < std::numeric_limits<G>::min() ||
                    height > std::numeric_limits<G>::max()) {
                    throw std::overflow_error("Cannot cast to target type");
                }
                return Size<G>(static_cast<G>(width), static_cast<G>(height));
            }
    ...
    struct Rect<T> {
        ...
        auto center() const
        {
            return (pos + Point<T>(pos.x + size.width, pos.y + size.height)) / 2;
        }
        template <typename G>
