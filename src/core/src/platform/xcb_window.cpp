...
{
    xcb_test_fake_input(_connection, XCB_MOTION_NOTIFY, 0, XCB_CURRENT_TIME, XCB_NONE, rootX, rootY,
                        0);
    flush();
}
...
{
    xcb_test_fake_input(_connection, XCB_BUTTON_RELEASE, button, XCB_CURRENT_TIME, XCB_NONE, 0, 0,
                        0);
    flush();
}
...
auto XcbConnect::query_extension(const char *name, int *majorOpcodeReturn, int *firstEventReturn,
...
auto XcbConnect::select_event(XcbWindow *window) -> void
{
    ...
    if ((attr.all_event_masks & XCB_EVENT_MASK_POINTER_MOTION) == XCB_EVENT_MASK_POINTER_MOTION) {
...
auto XcbWindow::_create_window(uint32_t eventMask) -> void
{
    ...
    xcb_change_window_attributes(_conn->connection(), _window, XCB_CW_OVERRIDE_REDIRECT, values1);
}
...
{
    auto cookie = xcb_get_property(_conn->connection(), 0, _window, atom, XCB_ATOM_STRING, 0, 1024);
    auto reply  = std::unique_ptr<xcb_get_property_reply_t, void (*)(xcb_get_property_reply_t *)>(
...
{
    auto cookie =
        xcb_change_window_attributes_checked(_conn->connection(), _window, attributeMask, values);
    std::unique_ptr<xcb_generic_error_t, void (*)(xcb_generic_error_t *)> reply(
        xcb_request_check(_conn->connection(), cookie),
        [](xcb_generic_error_t *ptr) { free(ptr); });
    if (reply) {
...
{
    auto cookie = xcb_configure_window_checked(_conn->connection(), _window, configMask, valueList);
    std::unique_ptr<xcb_generic_error_t, void (*)(xcb_generic_error_t *)> reply(
        xcb_request_check(_conn->connection(), cookie),
        [](xcb_generic_error_t *ptr) { free(ptr); });
    if (reply) {
